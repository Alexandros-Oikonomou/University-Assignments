...
void addToList(list** start, macaddress newMacaddress) {
  ...
  newList->next = (*start);
  (*start) = newList;
  fprintf(logsFile, "Pushed Mac address: %" PRIu64 ", Time scanned:%d\n", newMacaddress.mac, (int) newMacaddress.timestamp);
  fflush(logsFile);
}
...
void* testCov()  {
  while(1) {
    ...
    if (testCOVID()) {
      ...
      timespec_get(&ts, TIME_UTC);
      strftime(buff2, sizeof(buff2), "%D %T", gmtime(&ts.tv_sec));
      fprintf(logsFile, "Upload %d near contacts after possitive COVID test\nLocal date & time = %s.%09ld\n", getLength(closeContactsStart), buff2, ts.tv_nsec);
      fflush(logsFile);
      pthread_mutex_unlock(&mutex);
    }
  }
...
int main(void) {
  ...
  closeContactsFile = fopen("nearContacts.txt", "w");
