...
import gr.auth.ee.dsproject.node.Node92599260;
...
public class Creature implements gr.auth.ee.dsproject.pacman.AbstractCreature
{
  ...
  {
	ArrayList <Node92599260> Moves = new ArrayList <Node92599260>(4); // δημιουργία ArrayList
	Node92599260 A = new Node92599260(currPosition[0],currPosition[1],0,Maze); // δημιουργία αντικειμένων
	Node92599260 B = new Node92599260(currPosition[0],currPosition[1],1,Maze);
	Node92599260 C = new Node92599260(currPosition[0],currPosition[1],2,Maze);
	Node92599260 D = new Node92599260(currPosition[0],currPosition[1],3,Maze);
	
	Moves.add(A); // προσθήκη αντικειμένων στην ArrayList
	Moves.add(B);
	Moves.add(C);
	Moves.add(D);
	
	// εύρεση μεγαλυτερης τιμης αξιολόγησης
	int max = -1;
	double MaxEvaluation = -1000;
	for(int i = 0; i < Moves.size(); i++) {
		if(MaxEvaluation < Moves.get(i).getNodeEvaluation()) {
			MaxEvaluation = Moves.get(i).getNodeEvaluation(); // τιμη αξιολόγησης
			max = i; // ο αριθμός κίνησης της μεγαλύτερης τιμής αξιολόγησης
		}
	}
    ...
    return max; //επιστροφή της κίνησης
